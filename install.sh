...
readonly MIRROR_COUNTRY="CN"
readonly MIRRORLIST="/etc/pacman.d/mirrorlist"

# Fixed pacman mirrors, tried in the order listed. No reflector and no
# network-side ranking: the list is deterministic, so every install produces
# the same mirrorlist and a mirror outage cannot leave a half-ranked file
# behind. setup_mirrors() still probes each entry and drops the unreachable
# ones (some networks block individual mirrors), so the first *working* mirror
# becomes the primary.
readonly MIRROR_URLS=(
...
# -----------------------------------------------------------------------------
write_mirrorlist() { # write_mirrorlist URL...
    local url
    {
        printf '# Generated by %s - fixed Chinese mirrors, in priority order.\n' \
            "$PROJECT_NAME"
        printf '# Entries that did not answer during the install were dropped.\n'
        for url in "$@"; do
            printf '\n## %s\n' "$url"
...
# True when the mirror actually serves the core package database, which is the
# same request pacman -Sy makes. -f makes curl report an HTTP error (403/404)
# as a failure, so a mirror that answers but refuses us is not treated as alive.
mirror_is_reachable() {
    curl -fsS --max-time 10 --retry 1 -o /dev/null \
        "$1/core/os/x86_64/core.db"
}

setup_mirrors() {
    info "Configuring pacman mirrors (fixed Chinese mirrors)..."

    if [[ -e "$MIRRORLIST" ]]; then
        cp -a "$MIRRORLIST" "${MIRRORLIST}.iso-backup"
    fi

    # Probe the fixed list in order and keep only the mirrors that answer.
    # Without this, one unreachable entry would make "pacman -Sy" fail and abort
    # the install even though other mirrors in the list work fine.
    local reachable=() url
    for url in "${MIRROR_URLS[@]}"; do
        if mirror_is_reachable "$url"; then
            reachable+=("$url")
            ok "Mirror reachable: $url"
        else
            warn "Mirror unreachable, skipping: $url"
        fi
    done

    if (( ${#reachable[@]} == 0 )); then
        die "None of the configured Chinese mirrors is reachable."
    fi

    write_mirrorlist "${reachable[@]}"

    # pacstrap does not sync the package databases itself, and the ISO's copy
    # can be weeks old. Without this, pacstrap would resolve versions from the
    # stale database and then hit 404s against the freshly written mirrors.
    info "Syncing package databases..."
    pacman -Sy --noconfirm || die "Failed to synchronize package databases."
    ok "Package databases synchronized."

    ok "Mirrorlist written: $MIRRORLIST"
    ok "Country: $MIRROR_COUNTRY (${#reachable[@]}/${#MIRROR_URLS[@]} mirrors usable, primary: ${reachable[0]})"
}
